//! Purged out-of-sample cross-validation *in the selection fitness path* (QE-415).
//!
...
//! adjacent slices of the same series, not out-of-sample folds: a position opened in one window bleeds into
//! the next, so selection pressure rewarded in-sample fit. The leakage-free [`PurgedKFold`](crate::cv::PurgedKFold)
//! object existed but was unused in selection.
//!
//! [`purged_oos_fitness`] replaces that scalar with a **purged out-of-sample** estimate: the genome is scored
//! on each of the `k` disjoint [`PurgedKFold`] **test** folds **in isolation** (flat start), and the per-fold
//! return series are reduced to `mean ± SE` exactly as [`NoiseRobustFitness::from_windows`] does. Two
//! properties make this purged OOS:
//!
...
//! criterion. Fold construction is genome-independent, so [`oos_test_ranges`] is computed **once** and the
...
/// Default number of purged CV folds the selection fitness scores over (`≥ 2` for a real cross-validated
/// standard error). Mirrors [`DEFAULT_WINDOWS`](crate::backtest::DEFAULT_WINDOWS).
...
/// is what [`oos_test_ranges`] and the AC's `windows_disjoint` check operate on, so the geometry the fitness
/// consumes and the geometry the leakage-free invariant is proved on are one and the same.
#[must_use]
...
/// The `k` out-of-sample **test** ranges of `cv` over `0..n_bars`, in fold order. Genome-independent —
/// compute once before the search and reuse for every genome's [`purged_oos_fitness`].
#[must_use]
pub fn oos_test_ranges(cv: &PurgedKFold, n_bars: usize) -> Vec<Range<usize>> {
    cv.folds(n_bars).into_iter().map(|f| f.test).collect()
}

/// Purged out-of-sample fitness: backtest `genome` on each disjoint OOS `test_ranges` fold **in isolation**
/// (flat start) and reduce the per-fold net-of-cost return series to `mean ± SE`
...
mod tests {
    ...
    /// AC (a): an in-sample-overfit genome that is *great on the whole train window* but poor on the purged
    /// OOS folds must rank BELOW a genome that generalises across folds, under the new fitness.
    ///
    ...
    #[test]
    fn overfit_genome_ranks_below_robust_under_purged_oos() {
        let s = schema();
        let n = 120usize;
        ...
        // Purged OOS fitness over 4 folds: the overfit genome collapses (no trades in 3/4 folds) below the
        // robust genome, reversing the ranking the fix targets.
        let cv = selection_kfold(4, s.max_lookback(), DEFAULT_LABEL_HORIZON);
        let ranges = oos_test_ranges(&cv, n);
        let oos_overfit = purged_oos_fitness(&overfit, &bars, &ranges, &cfg()).mean;
        let oos_robust = purged_oos_fitness(&robust, &bars, &ranges, &cfg()).mean;
        assert!(
            oos_robust > oos_overfit,
            "purged OOS must demote the overfit genome: robust {oos_robust} !> overfit {oos_overfit}"
        );
    }
    ...
    fn selection_folds_are_leakage_free() {
        ...
        // partition the window (no gaps/overlaps) — every OOS bar is scored once.
        let ranges = oos_test_ranges(&cv, 200);
    ...
    fn fold_count_floored_and_short_window_is_neutral() {
        ...
        let empty = oos_test_ranges(&cv, 0);
        assert!(empty.is_empty());
        let s = schema();
        let neutral = purged_oos_fitness(&long_on(0, 2, 5_000), &[], &empty, &cfg());
