...
- **`enforce_kill(current_qty, mark, t) -> KillOutcome`** — the out-of-band halt action, driven purely by
  the `KillHandle` (no planner/cockpit target needed):
  - not tripped → `Live`.
  - tripped, first call → **flatten**: `plan_delta(Notional::ZERO, current_qty, mark)` yields the closing
    order (sell a long / buy a short), submitted **directly to the simulator** (the kill's *own* action, so
    it bypasses the submission halt); mark `flattened` → `Flattened(Some(fill))` (or `Flattened(None)` if
    already flat). The caller applies the returned fill to the keeper.
  - tripped, subsequent calls → `Halted` (position already flat; stay halted).
...
4. `flatten_when_already_flat_halts_without_an_order` — flat keeper → `enforce_kill` = `Flattened(None)` (no
   order submitted), still halted.
...
- **Flatten fills at the mark in sim.** The real venue fills the flatten order at market; the sim uses the
  ...
  detail; documented.
- **`enforce_kill` is caller-driven (once per tick).** The gate flattens on the first post-trip call and
  latches; a caller that never calls `enforce_kill` would still be **halted** on `submit`/`admit` (the halt
  is structural), but would not auto-flatten — the live loop calls `enforce_kill` each tick. Documented.
