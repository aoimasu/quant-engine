...
`Phase: PreP3` · `Area: runnable jobs / cli` · `Depends on: QE-129, QE-120, QE-105`
...
- Load+`verify()` the vintage (`qe-vintage`), read bars for the window (`qe-storage` `scan_bars`),
  run each chromosome through `qe_wfo::backtest::backtest`, weight-aggregate to ensemble returns.
- Compute the result contract (equity curve, drawdown, CAGR, Sharpe, Sortino, monthly returns) and
  write `result.json` into `--run-dir`.
- Emit **JSON-line progress** on stdout (`load/scan/simulate/report` → `done`), deterministic.
