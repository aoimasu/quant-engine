...
<a id="review-r2"></a>
# Review R2 — Quant-methodology hardening from the Max Dama panel (QE-43x)

> **Provenance.** A six-discipline panel — two senior quant researchers, two mathematicians, a senior
> software engineer, a trading expert — comparing *Max Dama on Automated Trading* against the engine on
> 2026-07-16 (independent analysis → debate → chair synthesis). The full report (verdicts, the sharpest
> debates, all 20 ranked recommendations) is
> [`docs/reviews/maxdama-vs-quant-engine.html`](./reviews/maxdama-vs-quant-engine.html); the **spec of
...
_All four delivered — see [`reviewed/`](mds/reviewed/) (qe-430..433)._

### R2.b — Medium / low priority (panel #5–#20, deferred triage)

_All sixteen delivered — see [`reviewed/`](mds/reviewed/) (qe-434..449)._
...
| Ticket | Title | Depends on | Status |
|--------|-------|------------|:------:|
| [QE-451](./architecture/qe-450-gp-indicator-evolution-design.md#4-design) | `evolve` GP engine + frozen-pool artifact — **epic COMPLETE (Phases 0 + 1a + 1b)**: seam proof → offline pool illumination → GP-aware deflation + tradability gates + K≤16 formula-pool freeze (default-off, no golden moved). See [`reviewed/qe-451-phase0.md`](./mds/reviewed/qe-451-phase0.md), [`qe-451-phase1a.md`](./mds/reviewed/qe-451-phase1a.md), [`qe-451-phase1b.md`](./mds/reviewed/qe-451-phase1b.md). **Carry-forward → QE-454: production seal MUST fold in the calibrated `N*` (`max(distinct, N*)`), not the analytic floor alone.** | ✅ | ✅ |
| [QE-452](./architecture/qe-450-gp-indicator-evolution-design.md#132-integration-surface-reuse-dont-rebuild) | Server: `evolve` run kind + formula-pool endpoints + separate pool lifecycle + firewall extension — **epic COMPLETE (Phases A + B)**: protocol + `qe-formula-pool` artifact + evolve CLI job → server read/governance routes + durable pool lifecycle (production-seal **fail-closed** until QE-454). See [`reviewed/qe-452-phaseA.md`](./mds/reviewed/qe-452-phaseA.md), [`qe-452-phaseB.md`](./mds/reviewed/qe-452-phaseB.md). | ✅ | ✅ |
| [QE-454](./architecture/qe-450-gp-indicator-evolution-design.md#136-sandbox-vs-production--three-independent-structural-barriers) | Ops-safety & governance: RBAC + 3 structural barriers + server-authoritative `seal_allowed` + tamper-evident audit + `GovernanceRecord` + run supervision — **epic COMPLETE (Phases A + B)**: Phase A (RBAC + hash-chained+HMAC audit + `GovernanceRecord`) + Phase B (`seal_allowed` predicate w/ 8 hard-blocks + `DEFLATION_BASIS_VERSION` + 3 barriers + N*/uncensored-PBO deflation fold-in + deadline/semaphore supervision). See [`reviewed/qe-454-phaseA.md`](./mds/reviewed/qe-454-phaseA.md), [`qe-454-phaseB.md`](./mds/reviewed/qe-454-phaseB.md). **Follow-up:** wire per-formula `gate_evidence` into the evolve pipeline to complete the live evolve→production-seal path (currently fail-closed by design). | ✅ | ✅ |
...
| [QE-469](./mds/tickets/QE-469.md) | CPCV-distributed OOS evidence — replace the single G1 terminal-holdout point estimate with a distribution of held-out Sharpe/DSR, reusing CSCV (`pbo.rs`) + `PurgedKFold` (`cv.rs`) | QE-113, QE-439, QE-467 | ✅ Delivered — [`reviewed/qe-469.md`](./mds/reviewed/qe-469.md) |
