...
| [QE-454](./architecture/qe-450-gp-indicator-evolution-design.md#136-sandbox-vs-production--three-independent-structural-barriers) | Ops-safety & governance: RBAC (`require_role`) + sandbox↔production (3 structural barriers) + server-authoritative `seal_allowed` + tamper-evident audit + `GovernanceRecord` + run supervision (deadline/semaphore/halt) — **epic, phased**: Phase A ✅ (RBAC + tamper-evident audit + `GovernanceRecord`, production seal fail-closed, see [`reviewed/qe-454-phaseA.md`](./mds/reviewed/qe-454-phaseA.md)), Phase B (seal predicate + 3 barriers + `DEFLATION_BASIS_VERSION` + N* deflation fold-in + supervision) pending  *(P1 — safety control for a dangerous capability)* | QE-452, QE-256, QE-439 | — |
