...
| [QE-452](./architecture/qe-450-gp-indicator-evolution-design.md#132-integration-surface-reuse-dont-rebuild) | Server: `evolve` run kind + formula-pool endpoints + separate pool lifecycle + firewall extension — **epic, phased**: Phase A ✅ (protocol + `qe-formula-pool` artifact + evolve CLI job, see [`reviewed/qe-452-phaseA.md`](./mds/reviewed/qe-452-phaseA.md)), Phase B (server read/governance routes + pool lifecycle) pending  *(P2)* | QE-451, QE-255, QE-257, QE-407 | — |
