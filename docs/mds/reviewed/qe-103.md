## QE-103 — Data-integrity & source reconciliation validation — PR #16 — [Approved]
...
### Re-review (post-approval follow-up — head `ca5a044`) → [Approved] (unchanged)

Both advisories resolved and **verified** via diff vs `bd1b81b`.
- **#1 (Gap/Hole convention trap).** `FillPlan.holes` is now `Vec<Hole>` — a **distinct** type with
  `from_ms` documented as the first *unfilled* slot (inclusive) and `missing = (to-from)/interval`, plus an
  explicit doc note contrasting it with `integrity::Gap` (`from_ms` = last present sample) and naming the
  QE-104 fuser as the consumer. `make_hole` returns `Hole`; exported as `qe_ingest::Hole`. This is the
  "give fill its own type" option I recommended — the cleanest fix. The computation is **unchanged** (only
  the return type renamed `Gap`→`Hole`), so leakage-safety and all fill tests are untouched; the downstream
  consumer can no longer confuse the two conventions.
- **#2 (duplicates count).** `SeriesIntegrity.duplicates` doc now states it lists each *occurrence*. Matches
  the code.

Purely additive (one new type + two doc edits); no AC behaviour changed — both ACs still hold as in the
initial pass. Same transparency caveat: the Rust toolchain is absent from this review environment, so the
gates were not executed here; a new type + doc edits are correct by inspection and consistent with the
developer-reported green run. Verdict: **[Approved]**.

### Merge

Squash-merged into `main` as `2c30300` (`QE-103: data-integrity & source reconciliation
validation (#16)`); remote branch `qe-103/data-integrity-reconciliation` deleted. All four required
CI checks (fmt/clippy/test/deny) were green and the PR was `MERGEABLE`/`CLEAN` at merge.
