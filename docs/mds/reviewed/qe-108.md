## QE-108 — Feature vector assembly → synthetic store — PR #21 — [Approved]
...
### Re-review (post-approval follow-up — head `2db9a6e`) → [Approved] (unchanged)

Verified via diff vs `dd2c314`. The substantive advisory is **properly fixed end-to-end**:
- **#1 (self-describing codec).** `to_bytes(&schema)` now prepends `[version u32][num_states u16][width
  u16]` before the `i64` time + `u16` slots, and `from_bytes(bytes, &schema)` reads that header and
  **returns `None` unless version, `num_states`, and width all match the reader's schema** (then reads
  from the post-header offset 16). `FeatureSchema` carries `num_states` (uniform across the catalogue).
  The bridge wires it correctly — `assemble_and_cache_features` encodes `v.to_bytes(&schema)` and
  `read_cached_feature` decodes `from_bytes(&b, &schema)`. So a same-lineage `num_states`/`CATALOGUE_
  VERSION` change at identical catalogue width can no longer silently mis-decode; it fails loudly to a
  cache miss. `decode_rejects_state_count_mismatch` (width-equal, states 5 vs 9 ⇒ `None`) is exactly the
  scenario I raised. The 8-byte header is deterministic, so reproducibility and batch==streaming parity
  are unchanged.
- **#2 (doc).** Codec docs rewritten to describe the real header validation; the vacuous "index >=
  sentinel" clause is gone.
- **#3 (flow density).** Correctly left as a documented caller contract (one `Sample`/bar + QE-104
  forward-fill); assembly-boundary enforcement belongs with QE-128 — accepted.

Both ACs still hold. Same transparency caveat: the Rust toolchain is absent from this review environment,
so gates were not executed here; the change (a header + validation + schema field + tests) is correct by
inspection and consistent with the developer-reported green run. Verdict: **[Approved]**.

### Merge

Squash-merged into `main` as `ccd3482` (`QE-108: feature vector assembly to synthetic store (#21)`);
remote branch `qe-108/feature-vector-assembly` deleted. All four required CI checks
