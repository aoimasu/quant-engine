## QE-011 — LMDB synthetic-data store — PR #11 — [Approved]
...
### Re-review (post-approval follow-up — head `495140f`) → [Approved] (unchanged)

Advisory #2 addressed and **verified**: `indicator_key_components_are_unambiguous` added in
`src/key.rs`. Confirmed at the diff level it is a **purely additive** test (+23 lines, one
`#[cfg(test)]` fn, **no production code touched**), so the prior approval stands by construction — it
only hardens it. The assertions are correct against the `indicator_key` layout: `("ab","c")` vs
`("a","bc")` differ in the leading `u16` length prefix (concatenation-collision guard); `lookback`
14≠21 and resolution `M5`≠`H1` each flip a distinct field; `time_from_key(&k1)` round-trips the trailing
`order(time)`. `qe-storage` unit tests now 6 (was 5). Advisory #1 (invalidate two-txn) intentionally
left as-is per guidance — correct under the single-writer contract. Same transparency caveat as the
initial pass: the Rust toolchain is absent from this review environment, so I could not execute the
gates myself; an additive, standard-formatted test does not perturb fmt/clippy/deny, and the
developer-reported "6 unit + 10 market + 6 synthetic / clippy clean / fmt ok / deny unaffected" is
consistent with the diff. Verdict: **[Approved]**.
...
Squash-merged into `main` as commit `a0037d9` (PR #11). Re-review of the advisory #2 follow-up
(head `495140f`) re-affirmed `[Approved]`; all four CI checks (fmt/clippy/test/deny) green on the PR;
mergeable/CLEAN. Branch `qe-011/lmdb-synthetic-store` deleted (local + remote).
