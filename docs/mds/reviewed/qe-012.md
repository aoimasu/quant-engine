## QE-012 — Instrument-universe configuration & point-in-time membership — PR #12 — [Approved]
...
### Re-review (post-approval follow-up — head `b624122`) → [Approved] (unchanged)

Advisory #1 addressed and **verified** via diff vs `cf9708d`. `Config::validate` now validates
`history.start` through the same strict, leap/per-month-aware `parse_iso_date` as the universe; the
weaker `is_iso_date` helper is **fully removed** (`grep` across `crates/` → none remaining, no dead
code). The `match` on `history.start` stays exhaustive and correct — `None if !max_available` →
required-error (unchanged), `Some(start)` → strict parse propagating only on error, `None => {}` — so
behaviour is identical for valid dates and strictly tighter for calendar-invalid ones (e.g. `2021-02-29`
now rejected at `history.start`, the intended consolidation). The replacement test
`history_start_uses_strict_calendar_validation` rejects `2021-02-29` (asserting `field ==
"history.start"`) and accepts the real leap day `2020-02-29` end-to-end through `Config::from_toml_str`.
Advisory #2 intentionally left as-is (developer rationale accepted). Same transparency caveat: the Rust
toolchain is absent from this review environment, so gates were not executed here; a small
behaviour-narrowing refactor that removes a helper and swaps one validator is correct by inspection and
consistent with the developer-reported green run. Verdict: **[Approved]**.

### Merge

Squash-merged into `main` as commit `80fd563` (PR #12). Re-review of the advisory #1 follow-up
(head `b624122`) re-affirmed `[Approved]`; all four CI checks (fmt/clippy/test/deny) green on the PR;
mergeable/CLEAN. Branch `qe-012/instrument-universe` deleted (local + remote).
